...
def decrypt_image():
    ...
    if decoded_image_data:
        ...
        save_path = filedialog.asksaveasfilename(initialfile=f"{file_name}.png", defaultextension=".png", filetypes=[("PNG files", "*.png")], title="Lưu file đã giải mã")
        if save_path:
            with open(save_path, 'wb') as decoded_file:
                decoded_file.write(decoded_image_data)
            messagebox.showinfo("Thành công", f"Hình ảnh đã được giải mã và lưu vào file {save_path}")

# Tạo cửa sổ
root = tk.Tk()
root.title("Giải mã hình ảnh")
...
# Tạo nút giải mã
decrypt_button = tk.Button(root, width=10, text="Giải mã", command=decrypt_image, font=("SF UI Display Condensed", 10, "bold"))
